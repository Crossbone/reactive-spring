...
public class Step8Context {
	...
	@Test
	public void verifySecurityFilter() {

		Mono<String> mono = Mono.just("Hello User %s");

		Mono<String> rendered = mono.flatMap(Step8Context.securityFilter("HAL", inner -> {
			return getUsername().map(user -> String.format(inner, user));
		}));

		rendered.as(StepVerifier::create) //
				.expectNext("Hello User HAL") //
				.verifyComplete();
	}

	private static <T> Function<T, Mono<? extends T>> securityFilter(String username,
			Function<? super T, ? extends Mono<? extends T>> transformer) {
		return t -> transformer.apply(t).subscriberContext(Context.of("username", username));
	}

	private static Mono<String> getUsername() {
		return Mono.subscriberContext().map(it -> it.get("username"));
	}
}
